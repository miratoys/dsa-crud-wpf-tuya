...
namespace Midterms
{
    public partial class MainWindow : Window
    {
        ...
        private ICollectionView shoppingsView;
        private ICollectionView cartItemsView;
        ...
        public MainWindow()
        {
            ...
            shoppingsView = CollectionViewSource.GetDefaultView(shoppings);
            cartItemsView = CollectionViewSource.GetDefaultView(cartItems);
            shoppingsView.SortDescriptions.Add(new SortDescription("ID", ListSortDirection.Ascending));
            cartItemsView.SortDescriptions.Add(new SortDescription("ID", ListSortDirection.Ascending));
            shoppingsView.Filter = ShoppingFilter;
            cartItemsView.Filter = ShoppingFilter;

            this.DataContext = this;
            cart.ItemsSource = shoppingsView;
            Addcart.ItemsSource = cartItemsView;
        }

        private bool ShoppingFilter(object item)
        {
            if (string.IsNullOrWhiteSpace(SearchBox.Text))
                return true;

            var shopping = item as Shopping;
            string keyword = SearchBox.Text.ToLower();

            return shopping.Name.ToLower().Contains(keyword) ||
                   shopping.Description.ToLower().Contains(keyword) ||
                   shopping.ID.ToString().Contains(keyword) ||
                   shopping.Price.ToString().Contains(keyword);
        }

        private void Search_TextChanged(object sender, TextChangedEventArgs e)
        {
            shoppingsView.Refresh();
        ...
        private void SortButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            shoppingsView.SortDescriptions.Clear();
            cartItemsView.SortDescriptions.Clear();

            var direction = sortAscending ? ListSortDirection.Ascending : ListSortDirection.Descending;
            shoppingsView.SortDescriptions.Add(new SortDescription("ID", direction));
            cartItemsView.SortDescriptions.Add(new SortDescription("ID", direction));

            ((Button)sender).Content = sortAscending ? "Sort by ID ↑" : "Sort by ID ↓";
        }
